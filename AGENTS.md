...
# Guilherme Frasão · RatosOS

Neste arquivo, "você" é o dono do sistema; as instruções são pro agente.

## 1. O que é este sistema

Workspace da marca pessoal e canal do YouTube de Guilherme Frasão (@guilherme.frasao), assinatura
"Bastidores de quem está construindo". O canal é sobre construção: negócios, liderança, vendas e
desenvolvimento pessoal, pra empreendedores, prestadores de serviço, gestores e gente ambiciosa
construindo carreira ou negócio. Aqui eu produzo roteiro, pauta, thumbnail, descrição e (mais pra
frente) proposta comercial. O que diferencia: experiência real documentada, sem estética de quem já
chegou. O guia completo do canal — posicionamento, pilares, formatos, referências, processo de
produção, critérios de qualidade — está em `_contexto/marca/guia-editorial-canal.md`.

**Regra deste sistema:** `conteudo/` é a bancada — roteiro em rascunho, thumbnail sendo testada,
descrição em edição. Vídeo publicado migra pra `publicacoes/<data>-<titulo>/` com o material final
(roteiro, thumbnail e descrição definitivos). `conteudo/` só guarda o que ainda não foi ao ar.
...
- `conteudo/` · produção em andamento: `roteiros/`, `thumbnails/`, `descricoes/`
- `projetos/` · projetos internos (ex: a agência, outras frentes)
- `estudos/` · pesquisa de referência, análise de outros canais
- `publicacoes/` · vídeos já publicados, uma pasta por vídeo (`AAAA-MM-DD-titulo/`)
